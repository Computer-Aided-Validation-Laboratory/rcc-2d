...
def write_camera_depths(
    float_path: Path, bit_depths: Iterable[int], *, overwrite: bool = False,
) -> None:
...
def save_float_and_depths(
    float_path: Path, image: np.ndarray, bit_depths: Iterable[int], *,
    ...
    if overwrite or not float_path.exists():
        np.save(float_path, np.ascontiguousarray(image, dtype=np.float64))
    write_camera_depths(float_path, bit_depths, overwrite=overwrite)
