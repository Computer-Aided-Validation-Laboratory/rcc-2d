...
from exp3_analysis_common import OUT, OS_RE, SS_RE, numeric_y_axis, parameter, pattern_of, release, title_lines

RESULTS=OUT/"exp3_analysis_gridmethod"

@dataclass(frozen=True)
class Record:
    case:str;root:str;config:str;directory:Path;ssaa:int;osamp:int;analytic:bool

def discover()->list[Record]:
    values=[]
    for directory in (OUT/"exp3_gridmethod").glob("*/*/*"):
        ...
        values.append(Record(case,root,config,directory,parameter(config,SS_RE),parameter(config,OS_RE),"_analytic_" in config))
    return values
...
def analyse(payload:tuple[Record,list[Record]])->list[dict[str,object]]:
    ...
    for frame in range(11):
        ...
        maximum,rms=plot(RESULTS/rec.case/rec.root/rec.config/f"frame{frame:02d}_difference.png",rec,ref,label,frame,*a,*b)
        rows.append({"Case":rec.case,"Root":rec.root,"Config":rec.config,"Frame":frame,"SSAA":rec.ssaa or 1,"OS":rec.osamp or 1,"Reference":label,"max_difference_px":maximum,"rms_difference_px":rms})
        del a,b;release()
    return rows

def convergence(rows:list[dict[str,object]])->None:
    groups=defaultdict(list)
    for row in rows:groups[(row["Case"],row["Root"],row["Frame"])].append(row)
    for (case,root,frame),values in groups.items():
        ...
        axis.set_xscale("log",base=2);numeric_y_axis(axis,plotted);axis.set_xlabel("SSAA samples along one pixel axis");axis.set_ylabel("max displacement difference [px]");axis.grid(alpha=.3);axis.legend(fontsize=8);axis.set_title(f"{title_lines(case+': '+root+' grid-method convergence')} | frame {frame:02d}\nReference: {title_lines(str(values[0]['Reference']))}",fontsize=9)
